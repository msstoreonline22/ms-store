...
const isItemEligibleForOffer = (item, offer) => {
  ...
  const itemProductId = normalizeId(item.productId || item.product);

  return offerProductIds.includes(itemProductId);
};

const calculateOfferDiscount = (cartItems = [], offer = null) => {
  if (
    !offer ||
    !offer.isActive ||
...
export function calculateCartTotals(
  ...
  const normalSubtotal = cartItems.reduce((total, item) => {
    return total + Number(item.price || 0) * Number(item.quantity || 0);
  }, 0);

  const offerResult = calculateOfferDiscount(cartItems, activeOffer);
